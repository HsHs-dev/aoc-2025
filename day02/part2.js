

const input = "7777742220-7777814718,3201990-3447830,49-86,653243-683065,91-129,24-41,1-15,2678-4638,1407-2511,221-504,867867-942148,1167452509-1167622686,9957459726-9957683116,379068-535983,757-1242,955118-1088945,297342-362801,548256-566461,4926-10075,736811-799457,1093342-1130060,620410-651225,65610339-65732429,992946118-993033511,5848473-5907215,17190619-17315301,203488-286290,15631-36109,5858509282-5858695889,87824047-87984031,1313113913-1313147594,795745221-795825571,46303-100636,4743038-4844422"

const ranges = input.split(',')


const isInvalid = (num) => {
  const numS = String(num)
  const numLen = numS.length

  // the highest fragment of a number you need to repeat is half the length
  for (let i = 1; i <= numLen / 2; i++) {

    const fragment = numS.substring(0, i)
    const repeats = numLen / i

    let build = "" 
    for (let j = 0; j < repeats; j++) {
      build += fragment
    }

    if (build === numS) {
      return true
    }

  }

  return false
}

let invalidIDs = 0
for (const range of ranges) {
  const idRange = range.split('-')
  const start = parseInt(idRange[0])
  const end = parseInt(idRange[1])

  for (let i = start; i <= end; i++) {

    // take the string part that it's length % current === 0
    // build a string out of it (repeat current.len / part.len times) and check if it equals current
    if (isInvalid(i)) {
      invalidIDs += i
    }
  
  }
}

console.log(invalidIDs)

